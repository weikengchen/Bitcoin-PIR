//! DPF-PIR WebSocket Server (Gen2)
//!
//! A WebSocket-based PIR server for browser clients (gen2 databases).
...
//! Plain WebSocket (ws://):
//! ```bash
//! cargo run --bin server2 -- --port 8092
//! ```
//!
//! Secure WebSocket (wss://) with TLS:
//! ```bash
//! cargo run --bin server2 -- --port 8092 --tls-cert /path/to/cert.pem --tls-key /path/to/key.pem
//! ```
//!
//! Small databases:
//! ```bash
//! cargo run --bin server2 -- --port 8092 --small
//! ```
//!
//! The server accepts WebSocket connections and processes PIR queries
//! using the same binary protocol as the TCP server.

use dpf_pir::server_config2::load_configuration2;
...
async fn main() {
    ...
    // Load configuration (databases are registered in server_config2.rs)
    let server_config = load_configuration2(args.small);

    let protocol = if tls_acceptor.is_some() { "wss" } else { "ws" };
    info!("Starting DPF-PIR Gen2 WebSocket server on port {} ({})", port, protocol);
    ...
    if server_config.registry.is_empty() {
        error!("No databases registered. Edit dpf_pir/src/server_config2.rs to add databases.");
        std::process::exit(1);
    }
