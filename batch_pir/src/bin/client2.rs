...
//!
//! Usage:
//!   cargo run --release -p batch_pir --bin client2 -- \
//!     --server0 ws://localhost:8093 --server1 ws://localhost:8094 \
//!     --hash <40-char hex script hash>

use batch_pir::eval::{self, DPF_N};
use batch_pir::protocol::{BatchQuery, Request, Response};
...
fn parse_args() -> Args {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" => {
                println!("Usage: client2 --hash <hex> [--server0 URL] [--server1 URL]");
                std::process::exit(0);
            }
            _ => {}
        }
